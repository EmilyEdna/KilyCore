    ...
    public class EnterpriseWebController : BaseController
    {
        ...
        public ObjectResultEx EditStockAttach(RequestEnterpriseGoodsStockAttach Param)
        {
            if (!string.IsNullOrEmpty(Param.BoxCodeNo))
            {
                ...
                foreach (var item in data)
                {
                    var res = Regex.Match(HttpUtility.UrlDecode(item), "http\\W{3}phone.cfda.vip/NewPhone/boxing.html\\?Id=[0-9a-fA-F]{8}(-[0-9a-fA-F]{4}){3}-[0-9a-fA-F]{12}&Code=(.*?)\\d{13}", RegexOptions.IgnoreCase).Value;
